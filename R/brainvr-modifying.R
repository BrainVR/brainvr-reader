...
#'
#' Operates on given BrainvrObject, as it needs to fix not only player log,
#' but potential goal positions in the object as well
...
translate_positions <- function(obj, offset) {
  obj <- transform_object(obj, "translate", translate_positions_df,
                          translate_positions_list, offset)
  return(obj)
}
...
mirror_axes <- function(obj) {
  obj <- transform_object(obj, "mirror", mirror_positions_df,
                          mirror_positions_list)
  return(obj)
}
...
resize_layout <- function(obj, multiplier) {
  obj <- transform_object(obj, "resize", resize_positions_df,
                          resize_positions_list, multiplier)
  return(obj)
}
