...
load_experiment <- function(folder, exp_timestamp = NULL,
  ...
  test_log <- open_experiment_logs(folder, exp_timestamp,
                                   flatten = TRUE)
...
open_experiment_infos <- function(directory, exp_timestamp = NULL,
  ...
  out <- open_brainvr_logs(directory, "ExperimentInfo",
    func = load_experiment_info, exp_timestamp, flatten
  )
  return(out)
}
...
load_experiment_log <- function(filepath) {
  ...
  ls$data <- read.table(filepath,
    header = TRUE, sep = ";",
    stringsAsFactors = FALSE, skip = bottomHeaderIndex,
    encoding = "UTF-8"
  )
...
load_result_log <- function(filepath) {
  ...
  df_data <- try(read.table(filepath,
    skip = bottom_index, sep = ";", header = TRUE,
...
#'
#' @examples
open_brainvr_logs <- function(directory, log_name, func, exp_timestamp = NULL,
                              flatten = FALSE) {
